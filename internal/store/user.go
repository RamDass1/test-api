...
func (s *Store) CreateUser(ctx context.Context, email, name, passwordHash string) (domain.User, error) {
	...
	res, err := s.q.ExecContext(ctx, q, email, passwordHash, name)
	if err != nil {
...
func (s *Store) UserByID(ctx context.Context, id int64) (domain.User, error) {
	...
	var u domain.User
	err := s.q.QueryRowContext(ctx, q, id).Scan(&u.ID, &u.Email, &u.Name, &u.CreatedAt)
	if err != nil {
...
func (s *Store) UserByEmail(ctx context.Context, email string) (domain.User, error) {
	...
	var u domain.User
	err := s.q.QueryRowContext(ctx, q, email).Scan(&u.ID, &u.Email, &u.Name, &u.CreatedAt)
	if err != nil {
...
func (s *Store) CredentialsByEmail(ctx context.Context, email string) (domain.Credentials, error) {
	...
	err := s.q.QueryRowContext(ctx, q, email).
